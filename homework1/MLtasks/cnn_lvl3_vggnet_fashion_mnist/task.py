"""
VGGNet on Fashion MNIST - CNN Image Classification Task
Implements VGGNet-style CNN with data loaders, augmentation, and evaluation.
"""
...
def get_task_metadata():
    ...
    return {
        'task_name': 'vggnet_fashion_mnist',
        'task_type': 'classification',
        'num_classes': 10,
        'input_shape': [1, 28, 28],
        'description': 'VGGNet-style CNN for Fashion MNIST classification'
    }
...
# -----------------------------
# MODEL (Modified): VGG-style CNN for 28x28 grayscale images
# -----------------------------
class VGGNetFashion(nn.Module):
    """
    Compact VGG-style network adapted for FashionMNIST (1x28x28).
    Uses VGG-like conv blocks with MaxPool, ending in a small classifier head.
    """
    ...
    def __init__(self, num_classes=10):
        ...
        self.features = nn.Sequential(
            conv_block(1, 64, 2),    # 28 -> 14
            conv_block(64, 128, 2),  # 14 -> 7
            conv_block(128, 256, 2), # 7 -> 3
            conv_block(256, 512, 2), # 3 -> 1
        )
...
# -----------------------------
# PREPROCESSING (Modified): slightly stronger augmentation for FashionMNIST
# -----------------------------
def make_dataloaders(batch_size=64, val_ratio=0.15, num_workers=2):
    """Create data loaders for Fashion MNIST with augmentation."""

    # Data augmentation for training (a bit stronger than LeNet baseline)
...
# -----------------------------
# BUILD MODEL (Modified): return VGGNetFashion instead of LeNet
# -----------------------------
def build_model(num_classes=10):
    """Build the VGG-style model."""
    model = VGGNetFashion(num_classes=num_classes)
    model = model.to(device)
    return model


# -----------------------------
# TRAINING / OPTIMIZER (Modified): use Adam (not AdamW)
# -----------------------------
def train(model, train_loader, val_loader, epochs=20, lr=0.001, weight_decay=1e-4):
    ...
    # Keep scheduler style the same (StepLR), but tuned a bit for VGG-style training
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            # Mild gradient clipping can stabilize deeper conv stacks
...
def main():
    """Main function to run the VGGNet training and evaluation."""
    print("=" * 60)
    print("VGGNet on Fashion MNIST - Image Classification Task")
    print("=" * 60)
    ...
    check1 = train_metrics['accuracy'] > 0.92
    status1 = "✓" if check1 else "✗"
    print(f"{status1} Train Accuracy > 0.92: {train_metrics['accuracy']:.4f}")
    checks_passed = checks_passed and check1

    # Check 2: Val accuracy > 0.92
    check2 = val_metrics['accuracy'] > 0.92
    status2 = "✓" if check2 else "✗"
    print(f"{status2} Val Accuracy > 0.92: {val_metrics['accuracy']:.4f}")
    checks_passed = checks_passed and check2

    # Check 3: Val loss < 0.30
    check3 = val_metrics['loss'] < 0.30
    status3 = "✓" if check3 else "✗"
    print(f"{status3} Val Loss < 0.30: {val_metrics['loss']:.4f}")
